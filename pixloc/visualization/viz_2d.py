...
def features_to_RGB(*Fs, skip=1):
    ...
    for F in Fs:
        c, h, w = F.shape
        F = np.rollaxis(F, 0, 3)  # 将通道轴调整到最后一个维度
        F = F.reshape(-1, c)    # 展平特征图
        flatten.append(F)
        shapes.append((h, w))
    flatten = np.concatenate(flatten, axis=0)# 将所有特征图堆叠在一起

    pca = PCA(n_components=3)
    # 如果skip大于1，则使用每隔skip个样本的子集进行PCA，加速计算
    if skip > 1:
        pca.fit(normalize(flatten[::skip])) # 对样本进行PCA计算
...
def plot_keypoints1(kpts, colors='lime', ps=6):
    """Plot keypoints for existing images.
    Args:
        kpts: list of ndarrays of size (N, 2).
        colors: string, or list of list of tuples (one for each keypoints).
    ...
    if not isinstance(colors, list):
        colors = [colors] * len(kpts)
    axes = plt.gcf().axes
    imgMask=cv2.imread("/home/lys/Workplace/datasets/test/mask_IMG_0117.JPG")
    imgMask=imgMask[:,:,0]
    for a, k, c in zip(axes, kpts, colors):
        if k is not None:
            a.scatter(k[:, 0], k[:, 1], c=c, s=ps, linewidths=0)
