...
class BaseRefiner:
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, T_init: Pose,
        ...
        dbid_to_p3dids = self.model3d.get_dbid_to_p3dids(p3did_to_dbids)  # 每张照片对应的所有点
        if multiscales is None:
            multiscales = [1]

        rnames = [self.model3d.dbs[i].name for i in dbid_to_p3dids.keys()]  # 取出参考图片的名字
        images_ref = [read_image(self.paths.reference_images / n)           # 读取所有参考图片
                      for n in rnames]
        ...
        for image_scale in multiscales:
            ...
            for idx, dbid in enumerate(dbid_to_p3dids):
                ...
                dbid_p3did_to_feats[dbid] = self.interp_sparse_observations(  # 每张照片中一个点在不同尺度的向量
                        features_ref_dense, scales_ref, dbid, p3dids)         # 和概率
                del features_ref_dense

            p3did_to_feat = self.aggregate_features(        # 一个点对应的特征
    ...
    # 车辆掩码mask融合实验
    def dense_feature_extraction3(self, image: np.array, name: str,
                                 image_scale: int = 1
                                 ) -> Tuple[List[torch.Tensor], List[int]]:
        features, scales, weight = self.feature_extractor(
                image, image_scale)
        #-------------------------------------------------------------
        mode='max'
        front='/home/lys/Workplace/datasets/jiawei_cheku_dif_time_outputs/query_dif_time_masks/mask_'
        back='.JPG'
        edgeName=name[15:-4]
        imgMask=cv2.imread(front+edgeName+back)
        if imgMask is not None:
            ...
            imgMask2=imgMask2.to(device)
            imgMask1=imgMask1.to(device)
            imgMask0=imgMask0.to(device)
            tem=torch.tensor(0, device='cuda:0', dtype=torch.float32)
            weight[0]=torch.where(imgMask2==1,tem,weight[0])
            weight[1]=torch.where(imgMask1==1,tem,weight[1])
            weight[2]=torch.where(imgMask0==1,tem,weight[2])
        #-------------------------------------------------------------
    ...
    # 边缘特征图的特征融合
    def dense_feature_extraction2(self, image: np.array, name: str,
                                 image_scale: int = 1
                                 ) -> Tuple[List[torch.Tensor], List[int]]:
        features, scales, weight = self.feature_extractor(
                image, image_scale)
        ...
        if("db" in name):
            ...
            newName=name[6:-4]
        edge_feature=np.load("/home/lys/Workplace/datasets/jiawei_cheku_outputs/features/"+newName+".npy") # (128, 384, 512)
        edge_feature_tensor=torch.Tensor(edge_feature)  # torch.Size([128, 384, 512])
        edge2=torch.unsqueeze(edge_feature_tensor,0). to(self.device)         # torch.Size([1, 128, 384, 512])
        edge1=torch.nn.functional.interpolate(edge2,size=(96,128),mode='bilinear'). to(self.device)   # torch.Size([1, 128, 96, 128])
        edge0=torch.nn.functional.interpolate(edge2,size=(24,32),mode='bilinear'). to(self.device)    # torch.Size([1, 128, 24, 32])
        features[0]=torch.cat([torch.squeeze(edge2,0),features[0]],0)   # torch.Size([160, 384, 512])
        # features[1]=torch.cat([torch.squeeze(edge1,0),features[1]],0)   # torch.Size([256, 96, 128])
        # features[2]=torch.cat([torch.squeeze(edge0,0),features[2]],0)   # torch.Size([256, 24, 32])
    ...
    # 边缘特征概率图的融合
    def dense_feature_extraction1(self, image: np.array, name: str,
                                 image_scale: int = 1
                                 ) -> Tuple[List[torch.Tensor], List[int]]:
        features, scales, weight = self.feature_extractor(
                image, image_scale)
        #-------------------------------------------------------------
        mode='max'
        front='/home/lys/Workplace/datasets/jiawei_cheku_outputs/edge_512/edge-'
        back='.JPG'
        edgeName=name[6:-4]
        image_edge=cv2.imread(front+edgeName+back)
        image_edge=image_edge[:,:,0]
        image_edge = image_edge.astype(np.float32)
        confresize=1024
        resize2=confresize//image_scale
        resize1=resize2//4
        resize0=resize1//4
        image_edge2,_=resize(image_edge, resize2,max, interp='nearest')
        image_edge1,_=resize(image_edge, resize1,max, interp='nearest')
        image_edge0,_=resize(image_edge, resize0,max, interp='nearest')
        ...
        image_edge1=np.where(image_edge1>0.7,1,image_edge1)
        image_edge0=np.where(image_edge0>0.7,1,image_edge0)
        image_edge2=torch.tensor(image_edge2)
        image_edge1=torch.tensor(image_edge1)
        image_edge0=torch.tensor(image_edge0)
        image_edge2=image_edge2.unsqueeze(0)
        image_edge1=image_edge1.unsqueeze(0)
        image_edge0=image_edge0.unsqueeze(0)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        image_edge2=image_edge2.to(device)
        image_edge1=image_edge1.to(device)
        image_edge0=image_edge0.to(device)
        if(mode=='max'):
            weight[0]=torch.where(weight[0]>image_edge2,weight[0],image_edge2)
            weight[1]=torch.where(weight[1]>image_edge1,weight[1],image_edge1)
            weight[2]=torch.where(weight[2]>image_edge0,weight[2],image_edge0)
        elif(mode=='avg'):
            weight[0]=(weight[0]+image_edge2)/2
            weight[1]=(weight[1]+image_edge1)/2
            weight[2]=(weight[2]+image_edge0)/2
        #-------------------------------------------------------------
    ...
    def aggregate_features(self,
        ...
        for p3id, obs_dbids in p3did_to_dbids.items():      # 筛选出共视点
            features = []
